const overlayCrop = document.getElementById("overlay-crop"),
  ...
  downloadBtnOptions = document.getElementById("download-btn-options"),
...
const readURL = (e) => {
  const isValid = allowedFormats.some((v) => v === input.files[0].type);

  if (input.files && input.files[0] && isValid) {
    // El objeto FileReader lee el archivo en el input
    // Lo devuelve en el formato que se le pida, en este caso una URL
    const reader = new FileReader();
    reader.onload = (e) => {
      overlayCrop.style.height = "100%";
      rect = {};
      // Setea el resultado que da el objeto FileReader en el atributo src
      myimg.src = e.target.result;
      // Por alguna razon necesita el setTimeout (aun sin tiempo) para setear bien las medidas
      // Luego le agrege un tiempo para esperar las animaciones
      timeoutId = setTimeout(() => {
        ...
        draw();
        // animaciones
        imageContainer.style.zIndex = "150";
        myimg.style.opacity = "1";
        canvas.style.opacity = "1";
        uploadBtn.classList.remove("btn-neon");
        uploadBtn.classList.add("btn-pulse", "browse");
        contactBtn.classList.add("btn-pulse", "contact-top");
        logo.classList.add("logo-top");
        openOptions.style.opacity = "1";
        downloadBtn.style.opacity = "1";
        cropBtn.style.opacity = "1";
        downloadBtn.classList.remove("pulse-effect");
        cropBtn.classList.add("pulse-effect");
...
function init() {
  ...
  myimg.style.maxWidth = `${imageContainer.scrollWidth}px`;
  myimg.style.maxHeight = `${imageContainer.scrollHeight}px`;
  ...
  downloadBtnOptions.addEventListener("click", donwloadImg, false);
  openOptions.addEventListener("click", openOptionsOverlay, false);
  closeOptions.addEventListener("click", closeOptionsOverlay, false);
}

function openOptionsOverlay() {
  overlayOptions.style.height = "0";
  overlayOptions.style.height = "100%";
}
...
function donwloadImg(e) {
  ...
  if (cropped) {
    ...
    e.currentTarget.setAttribute("download", name + "." + format);
...
function cropImg() {
  if (!cropped) {
    ...
    myimg.src = "";
    cropped = true;
    cropBtn.classList.remove("pulse-effect");
    downloadBtn.classList.add("pulse-effect");
  }
}
